...
export const DOMAIN_PLAN_STATUSES = ['Active', 'Inactive']
...
export const DOMAIN_PLAN_FORM_FIELDS = [
  ...
  { name: 'price', label: 'Monthly Price (₹)', type: 'number', placeholder: '0' },
  {
    ...
    label: 'Status',
    type: 'select',
    options: DOMAIN_PLAN_STATUSES.map((s) => ({ value: s, label: s })),
  },
  { name: 'description', label: 'Description', type: 'textarea', full: true },
]

export const DOMAIN_PLAN_FORM_DEFAULTS = {
  name: '', code: '', description: '', price: 0, status: 'Active',
}

export function useDomainPlanOptions(currentValue, { enabled = true } = {}) {
  const { data = [], isLoading } = useQuery({
    queryKey: [DOMAIN_PLAN_QUERY_KEY, 'options'],
